...
useReducer powerfull tool for complex state handling
deep equal in lodash
Specific helper functions should be extracted, passing to it a dispatcher, and provided through imports and not the Provider directly, as it will need a useCallback. We don't want that, let's import it so we don't need to care for it as props.
